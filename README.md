...
### <font color="red">Não verifiquei tudo!
# Pessoas
* <font color="orange">`ShortId`em pessoas, não está em uppercase.
* <font color="orange">Código alteranativo não está sendo salvo, ou melhor, está sendo salvo como `vazio`.
* <font color="orange">Há um erro na validação para alteração de pessoas.
* <font color="red"/>Creio que linha de validação, <font color="purple">`RuleFor(x => x.AlternativeIdentifier).MustAsync(AlternativeCodeDoesNotExist).WithMessage("The alternative Identifier given it's already on database.");`</font> está com a lógica errada, pois a finalidade seria evitar que não se cadastre mais de um <font color="purple">`AlternativeIdentifier`</font> iguais, mas, da forma que está não deixará fazer nenhuma alteração, pois após inclusão sempre haverá um registro igual.
* A mensagem de exception em <font color="purple">`UpdatePersonHandler`</font>, quando não consegue recuperar a pessoa, ela é muito genérica e não dá para poder saber o que deve ser feito. Ao tentar recuperar a pessoa para atualização. Se para atualizar eu sou obrigado a informar o <font color="purple">`ShortId`</font> ele deve ser validado e não ser vazio. 
* Outro ponto, que o <font color="purple">`ShortId`</font> para atualização ou vai no corpo da requisição no como uma variável de post, nos dois não pode.
</font>
